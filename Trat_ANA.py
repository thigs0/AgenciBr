...
class TratAna:
    ...
    def __init__(self, arquivo, sep=';', index_col=False, list=False, encoding='latin-1',
                 on_bad_lines='skip', virgula_to_ponto=True):
        def find_cod(df):
            '''
    ...
    def trat_dados_arimetica(self, path_estations, ano,
        ...
        indices = self.identificador_distancia(path_estations,start_alt=start_alt, stop_alt=stop_alt,
                                               len=len, limite_distancia=limite_distancia, return_distance=return_distance
                                               , alt=alt)
        # constantes
        var1 = 0
        cont = 1
        ...
        while k < numero_estacoes:  # Temos o indice da estação, pegamos o valor de mesma linha nele
            ...
            try:
                ...
                if type(indice_linha) == int:
                    ...
                    print('Erro, uma das estações não está como um numero inteiro')
                    cont += 1
        ...
        for linha in range(len(self.dataset)):
            for dia in range(1,32):
                ...
                try:
                    ...
                    prec.append(self.dataset[colum[dia]][linha])
                except:
                    n=0
    ...
    def to_netcdf4(self, local):
        ...
        df = df.rename(columns={'precipitação':'pr','Data':'time'})
        # pr para numero
        temp = []
        for k in df['pr']:
            temp.append(float(k))
    ...
    def rx5day(self, valor=50, retornar='consecutive_dry_days_index_per_time_period', selbyyear=0):
        '''
    ...
    def ecacdd(self, R=1, N=5):
        '''
        (Consecutive dry days index per time period ou índice de dias secos consecutivos por período)
        R: Valor real. Representa o limiar de precipitação em mm/dia.
        Todo o valor de precipitação menor que esse limiar será considerado. O valor padrão de uso é R = 1 mm/dia.
        ...
        self.only_mondata()
        ds = self.dataset
        consecutive_dry_days_index_per_time_period = {0: 0, 1: 0}
        lista = []
        maior = {0: 0, 1: 0}
        cont = 0
        temp = 0
        for linha in range(self.len):
            ...
            for dia in range(1, 32):
                try:
                    ...
                    if data.year == N:
                        # se o valor é menor do que desejamos
                        if float(ds.iloc[linha, dia]) < R:
                            temp += float(ds.iloc[linha, dia])
                            cont += 1

                        else:
                            # se obtivemos um período com menor chuva
                            if temp <= consecutive_dry_days_index_per_time_period[1]:
                                consecutive_dry_days_index_per_time_period[1] = temp
                                consecutive_dry_days_index_per_time_period[0] = (data + datetime.timedelta(days=-cont))
                            lista.append({'num': temp, 'month': (data + datetime.timedelta(days=-5))})
                            cont = 0
                            temp = 0

        return consecutive_dry_days_index_per_time_period


pt = []
meses =[]

lista_meses = ['/Medicoes_convencionais_Carauari_ANA/chuvas_T_00466001.txt','/Medicoes_convencionais_Carauari_ANA/chuvas_T_00668000.txt',
               '/Medicoes_convencionais_Manaus_ANA/chuvas_T_00260009.txt','/Medicoes_convencionais_Manaus_ANA/chuvas_T_00359005.txt',
               '/Medicoes_convencionais_Manaus_ANA/chuvas_T_00360005.txt']
for arquivo in lista_meses:

        lista = []

        arquivo = '/media/thigs/thigs/Projetos/Amazônia/selecionados/ANA'+arquivo

        ds = TratAna(arquivo, sep=';')
        ds.only_mondata()


        for k in range(ds.startdate.year, ds.enddate.year+1):
            try:
                print(ds.rx5day(50, retornar='highest_five_day_precipitation_amount_per_time_period', selbyyear=k))
                temp = ds.rx5day(50, retornar='highest_five_day_precipitation_amount_per_time_period', selbyyear=k)[1]
                lista.append(f'{temp.month}/{temp.year}')
            except:
                oi=1


        meses.append(lista[:])
        pt.append(arquivo[-35:-26] +arquivo[-12:-4])


print(len(meses), len(pt))
print('+=+'*30)
meses = completa(meses)
print(meses)
print('+=+'*30)
print(pt)
temp = pd.DataFrame(list(zip(meses[0],meses[1],meses[2], meses[3], meses[4])), columns=[f'{pt[0]}',f'{pt[1]}',f'{pt[2]}',f'{pt[3]}',f'{pt[4]}'])
temp.to_excel('tabela_teste.xlsx')
print(temp)
