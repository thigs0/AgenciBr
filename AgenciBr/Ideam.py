...
class Ideam():
    ...
    def empty_data(self):
        if self.type == 'format1':
            ...
            s = (s / self.len) * 100
            return s
        s = np.sum(np.isnan(self.dataframe[f'Valor'].to_numpy("float32")))
        s = (s / self.len) * 100
        return s
        ...
        if years != (0,0) and self.type_data != "format1": # if is not format1 and select date
            ...
            for i in range(len(a)):
                ...
                try:
                    ...
                    continue
            data = pd.date_range(start=datetime.date(self.startdate), end=self.enddate)  # change data to numpy datetime64

            if (float(years[0]) in pd.DatetimeIndex(data).year) and (
                    float(years[1]) not in pd.DatetimeIndex(data).year):  # The first date is in array and end date not
                ...
                if self.startdate != datetime(self.enddate.year, 1,
                                              1):  # complete missing days start, if don't start in 1/1
                ...
                if self.enddate != datetime(self.enddate.year, 12, 31):
                    ...
                    data = np.concatenate((data, pd.date_range(start=data[-1], end=f"31/12/{self.enddate.year}")))
